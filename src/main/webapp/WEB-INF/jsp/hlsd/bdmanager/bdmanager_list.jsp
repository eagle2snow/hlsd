			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="bdmanager/list.do" method="post" name="Form" id="Form">
							...
							<c:choose>
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											<td class='center'>
												<label class="pos-rel"><input type='checkbox' name='ids' value="${var.MANAGERID}" class="ace" /><span class="lbl"></span></label>
											</td>
											<td class='center' style="width: 30px;">${vs.index+1}</td>
											<td class='center'>${var.MANAGERID}</td>
											...
											<td class='center'>${var.UPDATER}</td>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<c:if test="${QX.edit == 1 }">
													<a class="btn btn-xs btn-success" title="编辑" onclick="edit('${var.MANAGERID}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
													</a>
													</c:if>
													<c:if test="${QX.del == 1 }">
													<a class="btn btn-xs btn-danger" onclick="del('${var.MANAGERID}');">
												...
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															<c:if test="${QX.edit == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="edit('${var.MANAGERID}');" class="tooltip-success" data-rel="tooltip" title="修改">
																	<span class="green">
															...
															<c:if test="${QX.del == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="del('${var.MANAGERID}');" class="tooltip-error" data-rel="tooltip" title="删除">
																	<span class="red">
	...
	<script type="text/javascript">
		...
		function add(){
			 top.layer.open({
				  ...
				  shade: 0.1,
				  maxmin: false,
				  content: '<%=basePath%>bdmanager/goAdd.do'
			 });
		}
		...
		function del(Id){
			var layindex = top.layer.confirm('确定要删除吗?',function(){
				$.ajax({
					type: "POST",
					url: "<%=basePath%>bdmanager/delete.do?MANAGERID="+Id+"&tm="+new Date().getTime(),
					cache: false,
		...
		function edit(Id){
			 top.layer.open({
				  ...
				  shade: 0.1,
				  maxmin: false,
				  content: '<%=basePath%>bdmanager/goEdit.do?MANAGERID='+Id
			 });
		}
			...
			var layindex = top.layer.confirm(msg,function(){
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							$.ajax({
								type: "POST",
								url: '<%=basePath%>bdmanager/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>bdmanager/excel.do';
		}
	</script>
