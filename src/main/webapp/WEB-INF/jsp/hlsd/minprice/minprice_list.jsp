			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="minprice/list.do" method="post" name="Form" id="Form">
							    	...
							    	<table>
										<tr>
											<td>
												<div class="nav-search">
													<span class="input-icon">
														<input type="text" placeholder="这里输入商品描述" class="nav-search-input" id="nav-search-input" autocomplete="off" name="keywords" value="${pd.keywords }" />
											...
											<td style="vertical-align:top;padding-left:2px;">
											 	<select class="chosen-select form-control" name="DRIVERTYPE" id="id" data-placeholder="请选择" style="vertical-align:top;width: 120px;">
												<option value="">配送员类型</option>
												<option value="2" <c:if test="${pa.DRIVERTYPE == '2'}">selected="selected"</c:if>>司机</option>
												<option value="5" <c:if test="${pa.DRIVERTYPE == '5'}">selected="selected"</c:if>>骑手</option>
											  	</select>
											</td>
										</tr>
									</table>
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							<thead>
								<tr>
									...
									<th class="center">起步价(元)</th>
									<th class="center">拼单失败起步价(元)</th>
									<th class="center">拼单成功起步价(元)</th>
									<th class="center">配送员类型</th>
									<th class="center">商品描述</th>
									<th class="center">第二单最低消费</th>
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'>${var.MINPRICE}</td>
											<td class='center'>${var.BMINPRICE}</td>
											<td class='center'>${var.PMINPRICE}</td>
											<td class='center'>
												<c:if test="${var.DRIVERTYPE == '2'}">司机</c:if>
												<c:if test="${var.DRIVERTYPE == '5'}">骑手</c:if>
											</td>
											<td class='center'>${var.GOODSDESCRIPTION}</td>
											<td class='center'>${var.SECONDORDERMINFEE}</td>
											<td class="center">
	...
	<script type="text/javascript">
		...
		function add(){
			 top.layer.open({
				  ...
				  shade: 0.1,
				  maxmin: false,
				  content: '<%=basePath%>minprice/goAdd.do'
			 });
		}
		...
		function del(Id){
			var layindex = top.layer.confirm('确定要删除吗?',function(){
				$.ajax({
					type: "POST",
					url: "<%=basePath%>minprice/delete.do?ID="+Id+"&tm="+new Date().getTime(),
					cache: false,
		...
		function edit(Id){
			 top.layer.open({
				  ...
				  shade: 0.1,
				  maxmin: false,
				  content: '<%=basePath%>minprice/goEdit.do?ID='+Id
			 });
		}
			...
			var layindex = top.layer.confirm(msg,function(){
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							$.ajax({
								type: "POST",
								url: '<%=basePath%>minprice/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>minprice/excel.do';
		}
	</script>
