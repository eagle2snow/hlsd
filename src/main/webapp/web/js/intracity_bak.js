...
//计算价格
function sumprice() {
    if (checkobj(orderObj)) {
        return false;
    }
    priceObj = Object.assign(pircedata1,pircedata2, md5KeyObj, loginObj);
    $.ajax({
        url: urlprice,
        method: 'POST',
        data: priceObj,
        ...
        success: function(res) {
            ...
            if (res.result) {
                ...
                $('#total').html(res.fee)
                $('#distance').html('里程:' + res.distance + res.distanceUnit)
                ...
                layer.msg(res.resp_msg, { icon: 1 });
            } else {
                // layer.close(indexload); 
                layer.msg(res.resp_msg, { icon: 5 });
            }
        }
    })
}
...
function addtoinfo(temp) {
   ...
   if ($('#from-address').val()==''||$('#from-address').attr("data-lng")==''||$('#from-address').attr("data-lat")==''||$('#fromnum').val()=='') {
    ...
    if(temp==1){
        ...
        pircedata1= {
        'priceList[0].isPickup': $('.select3 .type-active').attr("data_id"),
       ...
       orderto2 = {
        ...
        pircedata2= {
        'priceList[1].isPickup': $('.select3 .type-active').attr("data_id"),
    ...
    orderhtml +='<label for="" style="padding-left: 60px;">订单详细</label><div class="del-edit-btn"><a href="">编辑</a><a href="">删除</a></div></div>'
...
$('#pay').on('click', function() {
    ...
    if (addtoinfoflag==1) {
            addtoinfo(1)
            orderObj = Object.assign(orderto1,orderto2, orderObjfrom, md5KeyObj, loginObj);
            sumprice()
            addorder()
    }
