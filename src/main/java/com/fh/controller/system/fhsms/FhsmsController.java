...
public class FhsmsController extends BaseController {
	...
	@RequestMapping(value="/listMessageSendRecord")
	public ModelAndView listMessageSendRecord(Page page) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+" 查看发送短信列表");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		String keywords = pd.getString("keywords");				//关键词检索条件
		...
		pd.put("TYPE", 3);
		page.setPd(pd);	
		List<PageData>	varList = new ArrayList<PageData>(); //列出Fhsms列表
		if(pd.getString("dataBaseName").equals("1")){
			varList = commonService.datalistPageForSms(page); //查询老环境的
		}else{
			varList = fhsmsService.list(page);
		}
		mv.setViewName("system/fhsms/message_list");
		mv.addObject("varList", varList);
		mv.addObject("pd", pd);
		mv.addObject("QX",Jurisdiction.getHC());				//按钮权限
		return mv;
