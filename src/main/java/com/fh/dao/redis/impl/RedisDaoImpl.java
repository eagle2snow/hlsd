...
public class RedisDaoImpl extends AbstractBaseRedisDao<String, PageData> implements RedisDao{
	 ...
	 */
	@Override
	public boolean addString(final String key, final String value,final long seconds) {
		boolean result = redisTemplate.execute(new RedisCallback<Boolean>() {  
			public Boolean doInRedis(RedisConnection connection)  
					throws DataAccessException {  
				RedisSerializer<String> serializer = getRedisSerializer();  
				byte[] jkey  = serializer.serialize(key);  
				byte[] jvalue = serializer.serialize(value); 
				connection.setEx(jkey, seconds, jvalue);
				return connection.setNX(jkey, jvalue);  
			}  
		});  
		return result; 
	 ...
	 */
	@Override
	public String addMap(String key, Map<String, String> map,int expire) {
		 Jedis jedis = getJedis();
		 String result = jedis.hmset(key,map);
